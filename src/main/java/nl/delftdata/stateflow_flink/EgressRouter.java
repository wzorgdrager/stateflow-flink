...
public class EgressRouter extends ProcessFunction<EventOuterClass.Event, EventOuterClass.Route> {
    ...
    public EventOuterClass.Route route(EventOuterClass.Event event) {
        if (event.getEventTypeCase() == EventOuterClass.Event.EventTypeCase.REQUEST && event.getRequest() == EventOuterClass.Request.EventFlow) {
            ...
            throw new RuntimeException("Expected either an EventFlow Request event or Reply, but got " + event.toString());
        }
    }
